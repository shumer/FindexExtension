...
root = Path(os.environ['RUNNER_TEMP']) / 'finderpack-signing'
...
run('xcrun', 'notarytool', 'store-credentials', 'FinderPack-CI', '--keychain', str(keychain),
    '--key', str(key), '--key-id', os.environ['NOTARY_KEY_ID'], '--issuer', os.environ['NOTARY_ISSUER_ID'])
with open(os.environ['GITHUB_ENV'], 'a') as environment:
    environment.write('CODESIGN_IDENTITY=' + identity[1] + '\n')
    environment.write('FINDERPACK_NOTARY_KEYCHAIN=' + str(keychain) + '\n')
certificate.unlink()
key.unlink()
