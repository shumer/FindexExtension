...
xcrun notarytool submit "$work/FinderPack.zip" --keychain-profile "$profile" \
...
if [ "$status" != Accepted ]; then
  if [ -n "$identifier" ]; then
    xcrun notarytool log "$identifier" --keychain-profile "$profile" "$work/notary-log.json" || true
  fi
